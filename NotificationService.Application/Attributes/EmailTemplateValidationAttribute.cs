...
namespace NotificationService.Application.Attributes
{
    ...
    /// the <c>EmailTemplateId</c> must be specified.
    /// </summary>
    /// <remarks>
    /// The validation passes if:
    /// - Both <c>Subject</c> and <c>Body</c> are provided.
    /// - Or, <c>EmailTemplateId</c> is not null.
    /// </remarks>
    public class EmailTemplateValidationAttribute : ValidationAttribute
    {
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            var emailTemplateId = instance?.GetType().GetProperty("EmailTemplateId")?.GetValue(instance) as int?;

            if ((string.IsNullOrEmpty(subject) || string.IsNullOrEmpty(body)) && emailTemplateId == null)
            {
